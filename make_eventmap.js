...
const WANT = ["engine_ext", "engine_int", "turbine", "engine_ext_old", "engine_int_old", "engine_custom"];
const out = { events: {} };
    ...
    for (const sheet of ev.parameter_sheets || []) {
      for (const I of sheet.instruments) {
        ...
        layers.push({
          ...
          // every prop#4 (instrument gain) curve, multiplied at playback: authored as fade-in /
          // fade-out pairs that together form a trapezoid window
          curves: (I.automation || []).filter(a => a.property_enum === 4)
            .map(a => a.points.map(p => [p.x, p.y])),
        });
      }
    }
    if (layers.length) compact.events[name] = { layers, oneshots };
  }
